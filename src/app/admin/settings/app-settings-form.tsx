...
export function AppSettingsForm({ settings }: AppSettingsFormProps) {
  ...
  const form = useForm<SettingsFormValues>({
    ...
    defaultValues: {
      ...
      onlinePaymentsEnabled: settings.features.onlinePaymentsEnabled ?? true,
                    ...
                    render={({ field }) => (
                        <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                            <div className="space-y-0.5">
                                ...
                                <FormDescription>If disabled, all online payment options (Razorpay, QR Code, etc.) will be hidden.</FormDescription>
                            </div>
                            <FormControl>
                                <Switch checked={field.value} onCheckedChange={field.onChange} />
                            </FormControl>
